...
liftCompOp op (x:y:xs) =
    Just $ (if y `op` x then -1 else 0) : xs
...
cstackThen (("else", kelse):("if", kif):(c, kold):cstack) =
    Just ((c, knew):cstack)
    where knew = (transIf kif kelse) . kold
cstackThen (("if", kif):(c, kold):cstack) =
    Just ((c, knew):cstack)
    where knew = (transIf kif id) . kold
...
cstackUntil (("begin", kloop):(c, kold):cstack) =
    Just ((c, knew):cstack)
    where knew = transUntilLoop kloop . kold
cstackUntil _ = Nothing


-- helpers
transIf :: Transition -> Transition -> Transition
transIf kif kelse (i:is, d, o)
    = if i /= 0 then kif (is, d, o) else kelse (is, d, o)
transIf _ _ _ = underflow

transUntilLoop :: Transition -> Transition
transUntilLoop kloop state
    = case kloop state of
        (i:is, d, o) -> if i /= 0
                        then (is, d, o)
                        else transUntilLoop kloop (is, d, o)
        _            -> underflow
